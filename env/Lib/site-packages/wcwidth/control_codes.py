...
ILLEGAL_CTRL = frozenset(
    chr(c)
    for c in (
        list(range(0x01, 0x07))
        + list(range(0x10, 0x1B))  # SOH, STX, ETX (^C), EOT (^D), ENQ, ACK
        + list(range(0x1C, 0x20))  # DLE through SUB (^Z)
        + [0x7F]  # FS, GS, RS, US
        + list(range(0x80, 0xA0))  # DEL  # C1 control characters
    )
)
