...
_EMOJI_ZWJ_SET = (
    frozenset(cp for lo, hi in EXTENDED_PICTOGRAPHIC for cp in range(lo, hi + 1))
...
_CONTROL_CHAR_TABLE = str.maketrans(
    ...
    (
        ...
        + "\x7f"  # C0: NUL through US (including tab)
        + "".join(chr(c) for c in range(0x80, 0xA0))  # DEL  # C1: U+0080-U+009F
    ),
)
...
def wcwidth(
    wc: str, unicode_version: str = "auto", ambiguous_width: int = 1
...
def wcswidth(
    ...
    _wcwidth = (
        ...
        else lambda c: wcwidth(c, "auto", ambiguous_width)
    )
    ...
    while idx < end:
        ...
        if ucs == 0xFE0F and last_measured_idx >= 0:
            ...
            total_width += _bisearch(
                ord(pwcs[last_measured_idx]), VS16_NARROW_TO_WIDE["9.0.0"]
            )
        ...
        if ucs > 0xFFFF:
            ...
            # Fitzpatrick modifier: zero-width when following emoji base
                ...
                _FITZPATRICK_RANGE[0] <= ucs <= _FITZPATRICK_RANGE[1]
...
def width(
    ...
    *,
    ...
    if control_codes == "parse" and len(text) > _WIDTH_FAST_PATH_MIN_LEN:
        ...
        if "\b" not in text and "\t" not in text and "\r" not in text:
            ...
            if "\x1b" not in text or (
                not CURSOR_RIGHT_SEQUENCE.search(text)
                and not CURSOR_LEFT_SEQUENCE.search(text)
            ):
    ...
    last_measured_idx = (
        -2
    )  # Track index of last measured char for VS16; -2 can never match idx-1
    last_measured_ucs = -1  # Codepoint of last measured char (for deferred emoji check)
    last_was_virama = False  # Virama conjunct formation state
    conjunct_pending = False  # Deferred +1 for bare conjuncts (no trailing Mc)
    text_len = len(text)
    ...
    while idx < text_len:
        ...
        if ucs == 0xFE0F:
            if last_measured_idx == idx - 1:
                if _bisearch(
                    ord(text[last_measured_idx]), VS16_NARROW_TO_WIDE["9.0.0"]
                ):
                    current_col += 1
                    max_extent = max(max_extent, current_col)
            # VS16 preserves emoji context: last_measured_ucs stays as the base
...
def ljust(
    ...
    *,
    ...
    if text.isascii() and text.isprintable():
        ...
        text_width = width(
            text, control_codes=control_codes, ambiguous_width=ambiguous_width
        )
