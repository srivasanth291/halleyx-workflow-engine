...
class SequenceTextWrapper(textwrap.TextWrapper):
    ...
    def _split(self, text: str) -> list[str]:  # pylint: disable=too-many-locals
        ...
        for segment, is_seq in iter_sequences(text):
            if not is_seq:
                ...
                if (
                    segment.startswith("\x1b]")
                    and stripped_text
                    and not stripped_text[-1].isspace()
                ):
        ...
        while chunks:
            ...
            if hyperlink_state is not None:
                open_seq = _make_hyperlink_open(
                    hyperlink_state.url,
                    hyperlink_state.params,
                    hyperlink_state.terminator,
                )
            ...
            if chunks and self._width(chunks[-1]) > line_width:
                self._handle_long_word(chunks, current_line, current_width, line_width)
            ...
            # Only drop if there's actual whitespace text, not if it's only sequences.
            stripped_last = (
                self._strip_sequences(current_line[-1]) if current_line else ""
            )
            if (
                self.drop_whitespace
                and current_line
                and stripped_last
                and not stripped_last.strip()
            ):
            ...
            if current_line:
                ...
                no_more_content = (
                    ...
                    and len(chunks) == 1
                    and not self._strip_sequences(chunks[0]).strip()
                )
                if (
                    ...
                    while current_line:
                        ...
                        if lines:
                            ...
                            if self._width(prev_line) + placeholder_w <= self.width:
    ...
    def _track_hyperlink_state(
        self, text: str, state: _HyperlinkState | None
    ) -> _HyperlinkState | None:
        """
            ...
            if self.break_on_hyphens:
                ...
                if len(stripped) > space_left:
                    ...
                    if hyphen_pos > 0 and any(c != "-" for c in stripped[:hyphen_pos]):
                        ...
                        hyphen_end = self._map_stripped_pos_to_original(
                            chunk, hyphen_pos + 1
                        )
