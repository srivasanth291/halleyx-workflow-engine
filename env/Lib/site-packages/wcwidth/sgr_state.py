...
def _sgr_state_is_active(state: _SGRState) -> bool:
    ...
    return (
        ...
        or state.underline
        or state.blink
        or state.rapid_blink
        or state.inverse
        or state.hidden
        or state.strikethrough
        or state.double_underline
        or state.foreground is not None
...
def _sgr_state_update(state: _SGRState, sequence: str) -> _SGRState:
    ...
    for p in params:
        ...
        # Basic colors (30-37, 40-47 standard; 90-97, 100-107 bright)
            ...
            _SGR.FG_BLACK <= p <= _SGR.FG_WHITE
            or _SGR.FG_BRIGHT_BLACK <= p <= _SGR.FG_BRIGHT_WHITE
        ):
            state = state._replace(foreground=(p,))
        elif (
            _SGR.BG_BLACK <= p <= _SGR.BG_WHITE
